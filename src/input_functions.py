...
:Synopsis:
    General command line input functions. Adapted from
    the book "Begin to Code with Python" by Rob Miles,
...
def read_text_with_choices(prompt, required: bool, choices: list):
    '''
    Displays a prompt and reads in a string of text
    expecting a selection from a list of choices.
    Keyboard interrupts (CTRL+C) are ignored
    Returns the selected choice.
    '''
    choices_prompt = f"{prompt} {choices} : "
    while True:
        choice = read_text(choices_prompt)
        choice = choice.strip()
        if len(choice) == 0:
            if required:
                continue
...
def read_text_with_default(prompt, default: str):
    '''
    Displays a prompt and reads in a string of text
    expecting a non-empty string. If the user enters 
    an empty string then the default value is returned.
    Keyboard interrupts (CTRL+C) are ignored.
    Returns the entered value or the default string.
    '''
    prompt = f"{prompt} [{default}] : "
    choice = read_text(prompt)
    choice = choice.strip()
    if len(choice) == 0:
        choice = default
    # return the result
...
def yes_no(prompt):
    '''
    Displays a prompt and reads in a string of text
    expecting a yes or no response.
    Keyboard interrupts (CTRL+C) are ignored
    Returns True if response is yes, False if no
    '''
    yes_no_prompt = f"{prompt} (y/n): "
    while True:
        result = read_text(yes_no_prompt)
        result_length = len(result)
        if (result_length < 1):
            continue
        first_character = result[0]
        if first_character.lower() == 'y':
            result = True
            break
        elif first_character.lower() == 'n':
            result = False
            break
    # return the result
    return result
